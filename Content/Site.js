...
(function()
{
 var Global=this,Runtime=this.IntelliFactory.Runtime,Site,Samples,Set,WebSharper,List,SpeechUtterance,SpeechSynthesis,Client,Builder,T,Operators,Sample,document,Html,PageletExtensions,Default,Operators1,Unchecked,jQuery,EventsPervasives,Seq,window,JavaScript,Arrays,Utils,OperatorIntrinsics,SpeechSynthesisUtterance;
 Runtime.Define(Global,{
  Site:{
   Client:{
    All:Runtime.Field(function()
    {
     ...
     return Set.Create(List.ofArray([_op_BangPlus_8(SpeechUtterance.Sample()),_op_BangPlus_8(SpeechSynthesis.Sample())]));
    }),
   ...
   Samples:{
    ...
    Sample:Runtime.Class({
     Show:function()
     {
      ...
      url="http://github.com/intellifactory/websharper.webspeech/blob/master/Site/"+this.FileName;
   ...
   SpeechSynthesis:{
    Main:function(e)
    {
     var info,pre,srecog,x,arg00,startBtn,x1,arg001,m;
     info=Default.Div(List.ofArray([Default.Text("Speak loud and clearly.")]));
     pre=Default.Pre(List.ofArray([Default.Attr().NewAttr("style","width: 550px; height: 300px; overflow-y: scroll; display: block;")]));
     srecog=new(window.SpeecRecognition||window.webkitSpeechRecognition)();
     JavaScript.Log(srecog.grammars);
     srecog.lang="en-US";
     srecog.continuous=true;
     srecog.onresult=function(res)
     {
      var array;
      array=Arrays.map(function(a)
      {
       return Utils.ToArray(a);
      },OperatorIntrinsics.GetArraySlice(Utils.ToArray(res.results),{
       $:1,
       $0:res.resultIndex
      },{
       $:0
      }));
      Arrays.iter(function(e1)
      {
       return pre.set_Text(pre.get_Text()+e1.transcript);
      },Arrays.map(SpeechSynthesis.MaxConfidence(),array));
      pre.Body.scrollTop=pre.Body.scrollHeight;
      return;
     };
     x=Default.Button(List.ofArray([Default.Text("Start")]));
     arg00=function()
     {
      return function()
      {
     ...
     EventsPervasives.Events().OnClick(arg00,x);
     startBtn=x;
     x1=Default.Button(List.ofArray([Default.Text("Stop")]));
     arg001=function()
     {
      return function()
      {
     ...
     m=Default.Div(List.ofArray([info,pre,startBtn,x1]));
     return jQuery(e).append(m.Body);
    },
    MaxConfidence:Runtime.Field(function()
    {
     var projection;
     ...
     {
      return Arrays.maxBy(projection,array);
     };
    }),
    Sample:Runtime.Field(function()
    {
     return Samples.Build().Id("SpeechSynthesis").FileName("SpeechSynthesis.fs").Keywords(List.ofArray(["user media","speech synthesis","speech","speech-to-text"])).Render(function(e)
     {
      return SpeechSynthesis.Main(e);
     }).Create();
    }),
    Utils:{
     ToArray:function(a)
     {
      var helper;
      helper=function(n,acc)
      {
       return n===-1?acc:helper(n-1,Runtime.New(T,{
        $:1,
        $0:a.item(n),
        $1:acc
       }));
      };
      return Arrays.ofSeq(helper(a.length-1,Runtime.New(T,{
       $:0
      })));
     }
    }
   ...
   SpeechUtterance:{
    ...
    {
     var m;
     m=Default.Div(SpeechUtterance.MkField(window.speechSynthesis,new SpeechSynthesisUtterance()));
     return jQuery(e).append(m.Body);
    },
    MkField:function(sp,u)
    {
     var x;
     x=SpeechUtterance.Texts();
     return List.map(Runtime.Tupled(function(elem)
     {
      var text,lang,t,x1,arg00,b;
      text=elem[0];
      lang=elem[1];
      t=Default.TextArea(List.ofArray([Default.Attr().NewAttr("width","200"),Default.Text(text)]));
      x1=Default.Button(List.ofArray([Default.Text("Speak!")]));
      arg00=function()
      {
       return function()
       {
      ...
      EventsPervasives.Events().OnClick(arg00,x1);
      b=x1;
      return Operators1.add(Default.Div(List.ofArray([Default.Attr().NewAttr("style","margin: 5px; display: inline-block;")])),List.ofArray([t,Default.Tags().NewTag("br",Runtime.New(T,{
       $:0
      })),b]));
     }),x);
    },
    Sample:Runtime.Field(function()
    {
     return Samples.Build().Id("SpeechUtterance").FileName("SpeechUtterance.fs").Keywords(List.ofArray(["user media","speech utterance","speech","TTS"])).Render(function(e)
     {
      return SpeechUtterance.Main(e);
     }).Create();
    }),
    Texts:Runtime.Field(function()
    {
     return List.ofArray([["Hey! How are you doing?","en-US"],["Hey! How are you doing?","en-GB"],["Hallo! Wie geht's?","de-DE"],["Ciao! Come và?","it-IT"],["\u3053\u3093\u306b\u3061\u306f\u304a\u5143\u6c17\u3067\u3059\u304b","ja-JP"]]);
    })
   }
  }
 });
 Runtime.OnInit(function()
 {
  ...
  WebSharper=Runtime.Safe(Global.IntelliFactory.WebSharper);
  List=Runtime.Safe(WebSharper.List);
  SpeechUtterance=Runtime.Safe(Site.SpeechUtterance);
  SpeechSynthesis=Runtime.Safe(Site.SpeechSynthesis);
  ...
  JavaScript=Runtime.Safe(WebSharper.JavaScript);
  Arrays=Runtime.Safe(WebSharper.Arrays);
  Utils=Runtime.Safe(SpeechSynthesis.Utils);
  OperatorIntrinsics=Runtime.Safe(WebSharper.OperatorIntrinsics);
  return SpeechSynthesisUtterance=Runtime.Safe(Global.SpeechSynthesisUtterance);
 });
 Runtime.OnLoad(function()
 {
